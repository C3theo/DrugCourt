...
class EmployerForm(forms.Form):
    ...
    hire_date = forms.DateField(label='Hire Date', input_formats=['%m/%d/%y'])
    term_date = forms.DateField(label='Term Date', input_formats=['%m/%d/%y'])
...
class SanctionForm(forms.Form):
    sanction_date = forms.DateField(label='Sanction Date', input_formats=['%m/%d/%y'])
    sanction_level = forms.ChoiceField(label='Sanction Level', choices=[('1', 'Sanction 1')])
    sanction_notes = forms.CharField(label='Sanction Notes')
    from_date = forms.DateField(label='Jail Start Date', input_formats=['%m/%d/%y'])
    to_date = forms.DateField(label='Jail End Date', input_formats=['%m/%d/%y'])
...
class CourtPhaseForm(forms.Form):
    ...
    phase_start = forms.DateField(label='Phase Start', input_formats=['%m/%d/%y'])
    phase_end = forms.DateField(label='Phase End', input_formats=['%m/%d/%y'])
    complete = forms.BooleanField(label='Complete')
    freeze_start = forms.DateField(label='Freeze Start', input_formats=['%m/%d/%y'])
    freeze_end = forms.DateField(label='Freeze End', input_formats=['%m/%d/%y'])
    discharge = forms.CharField()
    

class CourtDateForm(forms.Form):
    court_date = forms.DateField(label='Phase Start', input_formats=['%m/%d/%y'])
...
class CourtFeeForm(forms.Form):
    transaction_date = forms.DateField(label='Transaction Date', input_formats=['%m/%d/%y'])
    ...
    submit_date = forms.DateField(label='Transaction Date', input_formats=['%m/%d/%y'])
...
class CourtRewardForm(forms.Form):
    ...
    court_id = forms.ChoiceField(label='Court ID', choices=[('1', 'Court ID 1')])
...
class CourtProposalForm(forms.Form):
    ...
    proposal_type = forms.ChoiceField(label='Proposal Type', choices=[('1', 'Proposal Type')])
    proposal_status = forms.ChoiceField(label='Proposal Status', choices=[('1', 'Proposal Status')])
    pass_start = forms.DateField(label='Pass Start', input_formats=['%m/%d/%y'])
    pass_end = forms.DateField(label='Approve End', input_formats=['%m/%d/%y'])
    approve_date = forms.DateField(label='Pass End', input_formats=['%m/%d/%y'])
    entered_by = forms.CharField(label='Entered By')
...
class DrugScreenForm(forms.Form):
    collect_date = forms.DateField(label='Collect Date', input_formats=['%m/%d/%y'])
...
class CommunityServiceForm(forms.Form):
    cs_date = forms.DateField(label='Service Date', input_formats=['%m/%d/%y'])
